...
namespace StarFisher.Office.Outlook
{
    internal class LuncheonInviteeListEmail : EmailBase
    {
        ...
        private static void AddAwardLuncheonInviteeListAttachment(ComObjectManager com, MailItem mailItem, IExcelFileFactory excelFileFactory,
            NominationList nominationList)
        {
            var attachments = com.Get(() => mailItem.Attachments);
            var fileName = $@"{nominationList.Year}{nominationList.Quarter.Abbreviation}_StarAwards_LuncheonInvitees.xlsx";
        ...
        private static void WriteRequest(HtmlNode content, IEnumerable<Person> luncheonPlanners, string quarter)
        {
            ...
            content.ChildNodes.Append(HtmlNode.CreateNode(
                $@"<p class=MsoNormal>Please find attached the spreadsheet with the invite list for the {quarter} Star Awards luncheon.</p>"));
        }
    }
}
