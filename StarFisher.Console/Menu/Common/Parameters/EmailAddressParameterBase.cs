...
namespace StarFisher.Console.Menu.Common.Parameters
{
    public abstract class EmailAddressParameterBase : ParameterBase<EmailAddress>
    {
        ...
        protected EmailAddressParameterBase(IGlobalAddressList globalAddressList, PersonName personName)
        {
            ...
            _derivedEmailAddress = personName?.DerivedEmailAddress ?? throw new ArgumentNullException(nameof(personName));
        ...
        {
            PrintInvalidArgumentMessage(@"That's not a valid HealthStream email address, or it's not in the global address list. Valid HealthStream email addresses are like matt.emerson@healthstream.com.");
        }
